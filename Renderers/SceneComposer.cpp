...
void SceneComposer::UpdateInk(Windows::Foundation::Collections::IVectorView<Windows::UI::Input::Inking::InkStroke^>^ strokes)
{
	...
	for (unsigned int i = 0; i < strokes->Size; i++)
	{
		...
		//color->mixColor(Windows::UI::ColorHelper::FromArgb(1, 240, 52, 52));
		//drawingAttributes->Color = color->toColor();
		//drawingAttributes->DrawAsHighlighter = currentAttributes->DrawAsHighlighter;
		//drawingAttributes->FitToCurve = currentAttributes->FitToCurve;
		//drawingAttributes->IgnorePressure = currentAttributes->IgnorePressure;
		//drawingAttributes->IgnoreTilt = currentAttributes->IgnoreTilt;
		//drawingAttributes->Size = currentAttributes->Size;
		//drawingAttributes->ModelerAttributes->PredictionTime = currentAttributes->ModelerAttributes->PredictionTime;
		//drawingAttributes->ModelerAttributes->ScalingFactor = currentAttributes->ModelerAttributes->ScalingFactor;
		//drawingAttributes->PenTip = currentAttributes->PenTip;
		//// drawingAttributes->PencilProperties->Opacity = currentAttributes->PencilProperties->Opacity;
		//drawingAttributes->PenTipTransform = currentAttributes->PenTipTransform;
...
void DrawingCanvas::SceneComposer::UnselectSceneObjects()
{
	assert(_inkRenderer != nullptr);
	assert(_deviceResources != nullptr);
